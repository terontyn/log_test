...
def add_to_buffer(chat_id, new_urls):
    with BUFFER_LOCK:
        if chat_id in FILE_BUFFER:
            if FILE_BUFFER[chat_id].get("timer"): FILE_BUFFER[chat_id]["timer"].cancel()
...
def send_max_message(chat_id, text, reply_markup=None):
    ...
    try: requests.post(f"{MAX_API_URL}/messages", params={"chat_id": chat_id}, json=body, headers=HEADERS, timeout=20)
    except: pass
    return None
...
def answer_max_callback(callback_id):
    ...
    try: requests.post(f"{MAX_API_URL}/answers", params={"callback_id": callback_id}, json={}, headers=HEADERS, timeout=5)
    except: pass
...
def build_edit_kb(doc_id):
    # ДОБАВИЛИ НЕДОСТАЮЩИЕ ПОЛЯ В МЕНЮ ИСПРАВЛЕНИЯ
    return {"inline_keyboard": [
        [{"text": "📍 Локация выгрузки", "callback_data": f"field:{doc_id}:unloading_address"}],
        [{"text": "🚚 Перевозчик", "callback_data": f"field:{doc_id}:carrier_name"}],
...
def handle_callback(chat_id, data, callback_id, mid):
    ...
    if data.startswith("menu_op:"):
        ...
        doc_id = int(data.split(":")[1])
        doc = get_doc(doc_id)
        edit_max_message(mid, f"🛠 **Выберите поле для исправления:**", reply_markup=build_edit_kb(doc_id))
        ...
        txt = prompt_map.get(field, "✍️ Введите новое значение:")
        prompt_mid = send_max_message(chat_id, txt)
        EDIT_STATE[chat_id] = {"doc_id": int(did), "field": field, "original_mid": mid, "prompt_mid": prompt_mid}
        ...
        errors = []
        if not ocr.get("carrier_name", {}).get("value"): errors.append("Перевозчик")
        if not ocr.get("unloading_address", {}).get("value"): errors.append("Локация выгрузки")
        if not ocr.get("operation_type", {}).get("value"): errors.append("Статус")
...
def process_update(update):
    ...
    if state and text and update_type != "bot_started":
        ...
        if state.get("original_mid"): edit_max_message(state["original_mid"], msg_text, reply_markup=build_main_kb(doc_id))
    ...
    urls = [a.get("payload", {}).get("url") for a in attachments if (a.get("type") == "image" or a.get("type") == "file") and a.get("payload", {}).get("url")]
    if urls: add_to_buffer(chat_id, urls)
